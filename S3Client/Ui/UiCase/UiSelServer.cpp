...
#define	SCHEME_INI_SELSERV				"UiSelServer.ini"//"选服务器.ini"
...
void KUiSelServer::Initialize()
{
	...
	AddChild(&m_Cancel);
	// AddChild(&m_LeftList);
	// AddChild(&m_RightList);
	// AddChild(&m_NameBigger);
	// AddChild(&m_NetStatus);
	// AddChild(&m_KingSoft);
...
void KUiSelServer::LoadScheme(const char* pScheme)
{
	...
	if (Ini.Load(Buff))
	{
		...
		// m_LeftList   .Init(&Ini, "LeftList");
		// m_RightList   .Init(&Ini, "RightList");
		// m_NameBigger   .Init(&Ini, "NameBigger");
		// m_NetStatus   .Init(&Ini, "NetStatus");
