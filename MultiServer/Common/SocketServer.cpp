		...
		while (!m_shutdownEvent.Wait(0))
		{
			...
			else if (waitResult == WAIT_OBJECT_0 + 1)
			{
				...
				while (!m_shutdownEvent.Wait(0) && m_acceptConnectionsEvent.Wait(0))
				{
					...
					SOCKET acceptedSocket = ::WSAAccept(m_listeningSocket, reinterpret_cast<sockaddr*>(const_cast<BYTE*>(pAddress->GetBuffer())), &addressSize, 0, 0);
...
void CSocketServer::Socket::Attach( SOCKET theSocket )
{
	...
	OnlineGameLib::COpaqueUserData::SetUserData( 0 );
