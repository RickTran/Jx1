...
void CGameClient::ProcessCommand( const OnlineGameLib::Win32::CIOBuffer *pBuffer)
{
	...
	if ( ok )
	{
		...
		if ( datalength != 1000 ||
			0 != memcmp( pData, "BEGIN", strlen( "BEGIN" ) ) ||
			0 != memcmp( pData + 1000 - strlen( "END" ), "END", strlen( "END" ) ) )
		{
			OutPutInfo( "Found invalid data package on " + GetTimeStamp() );

			OutPutInfo( _T("Package[length:") + ToString( wDataLen ) + _T("]\n") + DumpData( pData, datalength, 40 ) );			
		}
		// ====================  END  =======================
	}
	else
